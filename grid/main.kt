...
data class PlayerState(
    ...
    val gold: Int,
    val facing: Facing,
    val collectedHerbFromSource: Int
)
    ...
    private suspend fun processCommand(cmd:  GameCommand){
        when(cmd){
            ...
            is CmdInteract -> {
                ...
                when (obj.type){
                    ...
                    WorldObjectType.HERB_SOURCE -> {
                        ...
                        if (player.collectedHerbFromSource >= 3) {
                            _events.emit(ServerMessage(cmd.playerId, "Источник Herb уже пуст"))
                            return
                        }
            ...
            is CmdDashForward -> {
                val player = getPlayerData(cmd.playerId)
                val stepZ = -2
                val targetX = player.gridX
                val targetZ = player.gridZ + stepZ

                if (!isCellInsideMap(targetX, targetZ) || isCellBlocked(targetX, targetZ)) {
                    _events.emit(ServerMessage(cmd.playerId, "Dash невозможен, путь заблокирован"))
                    return
                }

                updatePlayer(cmd.playerId) { p ->
                    p.copy(gridZ = targetZ, facing = Facing.FORWARD)
                }
        ...
        addPanelSurface {
            ...
            Column {
                ...
                Row {
                    ...
                    Button("Dash Forward") {
                        server.trySend(CmdDashForward(hud.activePlayerIdFlow.value))
                    }
                }
